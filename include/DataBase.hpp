...
public:
    ...
    std::list<std::unique_ptr<rocksdb::ColumnFamilyHandle>>
     open(const std::vector<rocksdb::ColumnFamilyDescriptor> &descriptors);
    boost::unordered_map<std::string, std::string>
     getRows(rocksdb::ColumnFamilyHandle *family);
...
boost::unordered_map<std::string, std::string> 
DataBase::getRows(rocksdb::ColumnFamilyHandle *family)
{
    using namespace rocksdb;
